...
namespace GoddadyApiImplementation.Infrastructure
{
    public partial class Client
    {
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        public async Task<DomainAvailableBulkResultResponse> BulkCheckDomainAvailable(Domains.Requests.DomainAvailableBulk request)
        {
            CheckRequestValid(request);
            var client = GetBaseHttpClient();
            ...
            if (response.StatusCode.ToString() == "203")
                ...
                return new DomainAvailableBulkResultResponse
                {
                    ...
                    BulkDomain = await response.Content.ReadAsAsync<Domains.Responses.DomainAvailableBulkResponse>()
                };
        }
        /// <summary>
        ...
        /// <param name="XShopperId"></param>
        /// <returns></returns>
        public async Task<bool> CancelDomain(string request, string XShopperId = null)
        {
            CheckRequestValid(request);
            var client = GetBaseHttpClient();
            if (XShopperId != null)
                client.DefaultRequestHeaders.Add("X-Shopper-Id", XShopperId);
            var response = await client.DeleteAsync($"domains/{request}");
            await CheckResponseMessageIsValid(response);
            return response.IsSuccessStatusCode;
        }
        /// <summary>
